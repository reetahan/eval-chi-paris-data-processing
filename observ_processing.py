...
def main():
	...
	aerosol_res, skipped2 = parse_aerosols(aerosols_xl)
	print(len(skipped2))
...
def parse_text(txt):
	...
	for line in txt:
		...
		if(hr > ((minute_mark + 125/3) % 1000) or hr < last_hr):
			...
			if(day < 32):
				if(hour < 10):
					key_str = "2010-01-" + str(day) + " 0" + str(hour) + ":00:00"
				else:
					key_str = "2010-01-" + str(day) + " " + str(hour) + ":00:00"
			else:
				day = day % 31
				if(day < 10):
					if(hour < 10):
						key_str = "2010-02-0" + str(day) + " 0" + str(hour) + ":00:00"
					else:
						key_str = "2010-02-0" + str(day) + " " + str(hour) + ":00:00"
				else:
					if(hour < 10):
						key_str = "2010-02-" + str(day) + " 0" + str(hour) + ":00:00"
					else:
						key_str = "2010-02-" + str(day) + " " + str(hour) + ":00:00"

			val_dict = {"O3_SRF": avg_o3, "CO_SRF": avg_co, "NO_SRF": avg_no, "NOX_SRF": avg_nox}
...
def parse_timerh(time_rh_raw):
	...
	for i in range(1,time_rh.nrows):
		...
		key_str = str(datetime.datetime(*xlrd.xldate_as_tuple(key_str_raw, time_rh_raw.datemode)))
...
def parse_voc(voc_raw):
	...
	for i in range(2,voc.nrows):
		...
		key_str = str(datetime.datetime(*xlrd.xldate_as_tuple(key_str_raw, voc_raw.datemode)))

		'''
		to_skip = False
		for j in [1,2,3,4,6,9,10,11,13,14,16,17]:
			if(voc.cell_type(i,j) in (xlrd.XL_CELL_EMPTY, xlrd.XL_CELL_BLANK)):
				to_skip =  True
				break
		if(to_skip):
			skipped.add(key_str)
			continue
		'''
		C2H6 = np.nan if voc.cell_value(i,1) == '' else float(voc.cell_value(i,1)) 
		ETH = np.nan if voc.cell_value(i,2) == '' else float(voc.cell_value(i,2))
		PAR3 = np.nan if voc.cell_value(i,3) == '' else float(voc.cell_value(i,3))
		OLET_PAR = np.nan if voc.cell_value(i,4) == '' else float(voc.cell_value(i,4))
		PAR4 = np.nan if voc.cell_value(i,6) == '' else float(voc.cell_value(i,6))
		PAR5 = np.nan if voc.cell_value(i,9) == '' else float(voc.cell_value(i,9))
		PAR6 = np.nan if voc.cell_value(i,10) == '' else float(voc.cell_value(i,10))
		TOL = np.nan if voc.cell_value(i,11) == '' else float(voc.cell_value(i,11))
		XYL = np.nan if voc.cell_value(i,13) == '' else float(voc.cell_value(i,13))
		CH3OH = np.nan if voc.cell_value(i,14) == '' else float(voc.cell_value(i,14))
		ALD2 = np.nan if voc.cell_value(i,16) == '' else float(voc.cell_value(i,16))
		AONE = np.nan if voc.cell_value(i,17) == '' else float(voc.cell_value(i,17))
		PAR = 3*PAR3 + 4*PAR4 + 5*PAR5 + 6*PAR6 + OLET_PAR
		OLET = OLET_PAR

		val_dict = {'C2H6_SRF': C2H6, 'ETH_SRF': ETH, 'PAR_SRF': PAR, 'OLET_SRF': OLET,'TOL_SRF':TOL, 'XYL_SRF':XYL, 'CH3OH_SRF': CH3OH, 'ALD2_SRF':ALD2, 'AONE_SRF':AONE}
...
def parse_aerosols(aerosol_raw):
	...
	for i in range(1,aerosol.nrows):
		...
		key_str = str(datetime.datetime(*xlrd.xldate_as_tuple(key_str_raw, aerosol_raw.datemode)))
		...
		val_dict = {'Mass_bc': BC, 'Mass_oa': OA, 'Mass_nh4': NH4, 'Mass_no3': NO3, 'Mass_so4':SO4, 'TotalMassBulk':TotalMassBulk, 'MFBC':MFBC, 'MFOA':MFOA, 'MFNH4':MFNH4, 'MFNO3': MFNO3, 'MFSO4':MFSO4, 'D_alpha':D_alpha, 'D_gamma':D_gamma, 'chi_abd': Chi}
		res[key_str] = val_dict
