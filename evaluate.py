...
def display_metrics(results, save_metfile):
    ...
    if(len(save_metfile) > 0):
        f = open(save_metfile, "w+")
        f.write("Validation Results\n========\n")

        f.write('RMSE: ' + str(rmse_validate) + "\n")
        f.write('MAPE: ' + str(mape_validate) + '%\n')

        f.write("Testing Results\n========\n")

        f.write('RMSE: ' + str(rmse_test) + "\n")
        f.write('MAPE: ' + str(mape_test) + '%\n')
        f.close()
    else:
...
def display_plots(results, save_val_file, save_test_file):
    ...
    if(len(save_val_file) > 0):
        plt.savefig(save_val_file)
    else:
        plt.show()
    ...
    if(len(save_test_file) > 0):
        plt.savefig(save_test_file)
    else:
        plt.show()
...
def main():
    ...
    for fname in [file_val, file_obs]:
        '''
        if fname == file_train:
            pred = "train_pred"
            pred_str = train_str
    ...
    display_metrics(results, metric_outfile)
    display_plots(results, val_outfile, test_outfile)
